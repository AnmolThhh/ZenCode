...
import { TrendingUp, Clock, Coffee, Eye, Zap, Target, Activity } from 'lucide-react';
...
function EfficiencyRing({ focusPct }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 p-6 rounded-2xl flex-1"
      style={{
        background: 'rgba(24,24,27,0.85)',
        border: `1px solid ${color}25`,
      }}
    >
      ...
      <div className="relative flex items-center justify-center">
        <svg width={110} height={110} style={{ transform: 'rotate(-90deg)' }}>
          {/* Track */}
          <circle cx={55} cy={55} r={r} fill="none" stroke="rgba(63,63,70,0.4)" strokeWidth={10} />
          {/* Progress */}
          <circle
            cx={55} cy={55} r={r}
            fill="none"
        ...
        <div className="absolute flex flex-col items-center">
          <span className="text-2xl font-black" style={{ color, fontFamily: 'var(--font-mono)', lineHeight: 1 }}>
            {focusPct}%
          </span>
          <span className="text-[9px] text-zinc-600 tracking-widest" style={{ fontFamily: 'var(--font-mono)' }}>
            focus
          </span>
        </div>
      </div>
...
export default function StatsView({ sessions }) {
  ...
  const totalFocus = useMemo(() => sessions.reduce((a, s) => a + s.focus, 0), [sessions]);
  const totalBreak = useMemo(() => sessions.reduce((a, s) => a + s.break, 0), [sessions]);
  const totalStray = useMemo(() => sessions.reduce((a, s) => a + s.stray, 0), [sessions]);
  ...
  return (
    <div className="flex-1 flex flex-col items-center overflow-y-auto no-scrollbar">
      <div className="flex flex-col w-full max-w-2xl px-10 pt-10 pb-10 gap-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          <StatCard Icon={TrendingUp} label="Best Focus"    value={formatDuration(bestFocus)} color="#34d399" sub="single session" />
          <StatCard Icon={Clock}      label="Avg Focus"     value={formatDuration(avgFocus)}  color="#818cf8" sub="per session" />
          <StatCard Icon={Coffee}     label="Total Break"   value={formatDuration(totalBreak)} color="#f59e0b" sub={`${sessions.length} sessions`} />
          <StatCard Icon={Eye}        label="Total Stray"   value={formatDuration(totalStray)} color="#f87171" sub="tab-away time" />
        </div>
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 flex flex-col gap-4">
            ...
            >
              ...
              <div>
                <span className="text-4xl font-black" style={{ color: '#fbbf24', fontFamily: 'var(--font-mono)', lineHeight: 1 }}>
                  {streak}
                </span>
            ...
            >
              ...
              <div>
                <span className="text-2xl font-black" style={{ color: '#818cf8', fontFamily: 'var(--font-mono)', lineHeight: 1 }}>
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-[10px] text-zinc-600 tracking-widest mt-0.5" style={{ fontFamily: 'var(--font-mono)' }}>
          ...
          {chartData.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-44 gap-3">
              ...
              <p className="text-xs text-zinc-700 tracking-widest" style={{ fontFamily: 'var(--font-mono)' }}>
        ...
        {chartData.length > 0 && (
          ...
          >
            ...
            <div className="flex items-center gap-5">
              {[['#34d399', 'Focus'], ['#f59e0b', 'Break'], ['#f87171', 'Stray']].map(([c, l]) => (
                <div key={l} className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full" style={{ background: c }} />
                  <span className="text-[10px] text-zinc-500 tracking-wider" style={{ fontFamily: 'var(--font-mono)' }}>{l}</span>
                </div>
              ))}
            </div>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart
                ...
                barGap={2}
              >
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(63,63,70,0.25)" vertical={false} />
                <XAxis
                  dataKey="idx"
                  tickFormatter={(v) => chartData[v]?.shortLabel ?? ''}
                  tick={{ fill: '#52525b', fontSize: 9, fontFamily: 'var(--font-mono)' }}
                ...
                <Bar dataKey="focus" fill="#34d399" radius={[3, 3, 0, 0]} maxBarSize={28} />
                <Bar dataKey="break" fill="#f59e0b" radius={[3, 3, 0, 0]} maxBarSize={28} />
                <Bar dataKey="stray" fill="#f87171" radius={[3, 3, 0, 0]} maxBarSize={28} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-xs font-semibold" style={{ color: '#34d399', fontFamily: 'var(--font-mono)' }}>
                    ...
                    {/* Progress bar */}
                    <div className="flex h-1.5 rounded-full overflow-hidden gap-px" style={{ background: 'rgba(63,63,70,0.3)' }}>
                      <div style={{ width: `${pct(s.focus, total)}%`, background: '#34d399', transition: 'width 0.6s ease' }} />
                      <div style={{ width: `${pct(s.break, total)}%`, background: '#f59e0b', transition: 'width 0.6s ease' }} />
                      <div style={{ width: `${pct(s.stray, total)}%`, background: '#f87171', transition: 'width 0.6s ease' }} />
                    </div>
                  </div>
                );
              })}
